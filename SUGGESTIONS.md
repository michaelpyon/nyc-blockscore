...
- Live <title> is the old "BlockScore: NYC Apartment Hunting Intelligence"
  copy while HEAD ships "BlockScore: NYC Block
...
1. **Evangelist (anxious renter):** I can search by address or paste a
   ...
   a curated 51 block sample. Compare picks survive a refresh. The
...
1. **Neighborhood filter on home (S, deploy Y)**: done this pass.
   Files: src/components/BlockGrid.tsx. 51 blocks is too many to scan;
...
6. **Per-compare OG card (M, deploy Y)**: DONE. The dynamic 2-up
   scorecard at /compare/opengraph-image mirrors the on-page verdict.
...
- Per-compare dynamic OG (item 6): completed in the AAA relaunch.
