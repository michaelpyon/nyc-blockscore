...
- Per-compare dynamic OG (route exists at src/app/compare/opengraph-image/route.tsx, currently uncommitted on main): 2-up scorecard PNG so a pasted compare link unfurls into the verdict on X and iMessage. This is the growth loop; treat as launch-blocking polish, verify with a real card validator before posting
...
- **Current truth**: all 52 blocks are curated SAMPLE data (src/data/seed.ts), realistic but illustrative. No live 311, DOB, DOHMH, or GTFS feeds are wired. Turso is an optional store, not a live pipeline
...
3. Per-compare dynamic OG card (route drafted, finish and commit)
4. 12-month sparkline per dimension on block detail

## Build notes

- Stack: Next.js 16.2.2, TS, Tailwind v4 tokens in src/app/globals.css (the token source; the old token list previously in this file is superseded by globals.css plus BRAND.md)
